...
def annotate():
    ...
    # define parameters
    bucket = 'gas-inputs'
    key = 'atinn/'+str(ID)+'~${filename}'
    if '${filename}' == '':
        file_does_not_exist()

    conditions = [{"acl": "public-read"},
                  ["starts-with", "$success_action_redirect","http://atinn-hw3.ucmpcs.org:5000/annotate/files"],
    ...
    signature = s3['fields']['signature']
    AWSaccesskeyid = s3['fields']['AWSAccessKeyId']

    return render_template("annotate.html", bucket_name=bucket, key=key, acl='public-read', policy=policy, signature=signature, aws_key=AWSaccesskeyid)
